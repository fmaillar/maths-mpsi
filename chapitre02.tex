...
\begin{proof}[Unicité]
    S'il existe deux couples de réels \((x, y),(x', y')\) tels que \(x' + \ii y' = x 
     + \ii y\), alors \(\underbrace{(x - x')^2}_{\geqslant 0} = 
...
\(M\), le complexe \(z = x + \ii y\). pour tout complexe \(z\), il existe un point 
...
\begin{proof}
    ...
    \in \intervalleentier{0}{n-1}}\), on déduit l'autre inclusion en vérifiant que tous les éléments de 
    \(\enstq{\expc{}{\frac{2k\pi}{n}}}{k \in \intervalleentier{0}{n - 1}}\) élevés à la puissance \(n\) 
    font 1.
\end{proof}
