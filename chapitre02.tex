\chapter{Nombres complexes}\label{chap:complexes}
\minitoc%
\minilof%
\minilot%
...
\begin{figure}
  ...
  \caption{Représentation géométrique des complexes}\label{fig:complexe}
\end{figure}

\begin{proof}[Unicité]
  S'il existe deux couples de réels \((x, y),(x', y')\) tels que \(x' + \ii{}y'
  = x + \ii{}y\), alors \(\underbrace{{(x - x')}^2}_{\geqslant 0} =
  \underbrace{-{(y'-y)}^2}_{\leqslant 0}\).
  Finalement \({(x - x')}^2 = {(y' - y)}^2 = 0\).
\end{proof}
...
\end{proof}
On appelle le réel \(x\) la partie réelle du complexe \(z\), \(x = \Re(z)\) et
on appelle le réel \(y\) la partie imaginaire du complexe \(z\), \(y = \Im(z)\).

\begin{prop}
  Soient des complexes \(z\) et \(z'\) puis un réel \(\lambda\), alors
  \begin{equation}
    \Re(z + z') = \Re(z) + \Re(z'), \quad \Im(z + z') = \Im(z) + \Im(z'),
    \quad \Re(\lambda z) = \lambda \Re(z), \quad \Im(\lambda z) = \lambda
    \Im(z).
  \end{equation}
\end{prop}
\emph{Un complexe n'a pas de signe}. On note \(\C^* = \C \setminus \{0\}\)

\subsection{Conjugaison de complexe}\label{subsec:conjugaisoncomplexe}
...
\subsection{Module d'un complexe}\label{subsec:modulecomplexe}
...
\section{Résolution d'équations du second degré}\label{sec:resolutionequationseconddegre}
...
\subsection{Application inversion}\label{subsec:appinverse}
...
\subsection{Barycentre}\label{subsec:complexebarycentre}
...
\subsection{Orthogonalité}\label{subsec:complexeorthogonalite}
...
\subsection{Alignement}\label{subsec:complexealignement}
...
\begin{exercice}[Méthode de Cardan]
  ...
  \begin{enumerate}
    ...
    \item Soit \(y\) une racine de l'équation~\eqref{eq:cardan1}. Montrer
      qu'il existe un couple \((u,v)\) de complexes tel que \(u + v = y\)
      et \(uv = -p/3\), et que \(u^3\) et \(v^3\) sont racines de
