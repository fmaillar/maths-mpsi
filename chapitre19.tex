...
Si \(F \in \K(X)\), un couple  \((A,B) \in \K[X]\times \K[X]\set minus\{0\}\) tel que \(F=\frac{A}{B}\) est appelé un représentant de \(F\). Il n'y a pas unicité de \((A,B)\).
...
\begin{proof}
  ...
  \(\impliedly\) S'il existe un scalaire \(\lambda\) non nul tel que \(\begin{cases} A_2=\lambda A_1 \\ B_2 = \lambda B_1 \end{cases}\) alors \(A_1B_2=\lambda A_1B_1=A_2B_1\) et donc \(\frac{A_1}{B_1}=\frac{A_2}{B_2}\).
\end{proof}
