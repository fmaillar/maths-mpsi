...
\begin{proof}
  ...
  \begin{align}
    ...
    & \leq \max(\deg(AB),\deg(BC))-\deg(B)-\deg(D) \\
    & \leq \max(\deg(A)+\deg(D),\deg(B)+\deg(C))-\deg(B)-\deg(D)\\
    & \leq \max(\deg(A)-\deg(B),\deg(C)-\deg(D))\\
    & \leq \max(\deg(F),\deg(G));
  \end{align}
  et pour le produit
  \begin{align}
...
\begin{proof}[Unicité]
  ...
  \end{align}
  avec $\deg\left(\sum_{i=1}^n\alpha_i(X-a)^{n-i}\right)\leq n-1 <n$. D'après l'unicité du lemme, on a forcèment
  \begin{equation}
...
Soit $P \in \C[X]\setminus\C$, $\deg(P)\geq 1$. Le polynôme $P$ est scindé donc il existe $n \in \N^*$, $\alpha_1, \ldots, \alpha_n$ des complexes deux à deux distincts, $k_1,\ldots, k_n$ des entiers naturels non nuls et un complexe non nul $\lambda$ tels que
\begin{equation}
  P = \lambda \prod_{i=1}^n (X-a_i)^{k_i}
\end{equation}
 Comme $\deg(P)\geq 1$, on a $\deg(P')=\deg(P)-1$. Alors $\deg\left(\frac{P'}{P}\right)=-1<0$. La partie entière de $frac{P'}{P}$ est nulle. Il existe de façon unique des complexes $\alpha_{i,j}$ tels que
 \begin{equation}
