...
\begin{proof}
  ...
  \end{equation}
  Or $\forall k>p \ a_k=0$ et $\forall k \leq p$ on a $q=r-p <n-p \leq n-k \leq n$ donc $n-k>q$ et $b_{n-k}=0$. Ainsi
  \begin{equation}
...
Cependant ce n'est pas un sous anneau dès que $n \geq 1$. Puisque $X^n \in \K_n[X]$ mais $X^n \cdot X^n = x^{2n} \notin \K_n[X]$.
...
\begin{proof}
  ...
  Soit $(a_0,\ldots, a_n) \in \K^{n+1}$ tel que $\sum_{k=0}^n a_k X^k =0$ alors pour tout $k$ tel que $0\leq k \leq n$ on a $a_k=0$ (un polynôme est nul si et seulement si ses coefficients sont nuls). La famille $\B$ est donc libre.
...
\end{prop}
\begin{proof}
  Soit $J=\{i_1, \ldots, i_k\} \subset I$. Quitte à réindexer, on peut supposer que $\deg(P_{i_1}) \leq \ldots \leq \deg(P_{i_k})$. Soit $k$ scalaires $\alpha_{i_1}, \ldots, \alpha_{i_k}$. On suppose que $\sum_{i \in J} \alpha_i P_i=0$. Supposons que cette famille est liée, c'est-à-dire qu'il existe $l \in J$ tel que $\alpha_l \neq 0$. Soit la partie $\{j \in J, \alpha_{ij} \neq 0\}$. Elle est incluse dans $\N$ et elle est non vide et elle est finie (parce que $J$ est fini). Alors elle admet un plus grand élément $j_0$. Ainsi $\alpha_{ij_0} \neq 0$.
  \begin{gather}
  ...
  \begin{equation}
    \deg(P_{ij_0}) \leq \max\limits_{1 \leq j \leq j_0-1}(\deg(P_{ij})) < \deg(P_{ij_0}).
  \end{equation}
...
\begin{proof}
  ...
  \begin{equation}
    \deg(P) \geq \deg\left(\prod_{i=1}^{n+1} (X-\alpha_i)\right) = n+1
  \end{equation}
...
\end{equation}
alors c'est une partie de $\N^*$ non vide (puisque $1 \in \courbe$) et majorée par le degré de $P$. En effet si $n \in \courbe$ alors $(X-\alpha)^n \mid{} P$ et en passant au degré on a $\deg(X-\alpha)^n \leq \deg P$ et donc $n \leq \deg(P)$. Ainsi $\courbe$ admet un plus grand élément. D'où la définition
...
\begin{proof}
  ...
  \end{itemize}
  D'après le corollaire~\ref{cor:tresImportant}, en comptant les racines avec leurs ordres de multiplicité
  \begin{equation}
    \sum_{i=1}^p r_i \geq n+1
  \end{equation}
  On a montré que $\prod_{i=1}^p (X-\alpha_i)^{r_i}$ divise $P$ donc en passant au degré
  \begin{equation}
    \deg(P) \geq \deg\left(\prod_{i=1}^p (X-\alpha_i)^{r_i}\right) = \sum_{i=1}^p r_i \geq n+1
  \end{equation}
\end{proof}
...
\begin{defdef}
  Soit $P = \sum_{n \geq 0}a_n X^n$ un polynôme à coefficients dans $\K$. On définit le polynôme dérivée, noté $P'$, par
  \begin{align}
    P' &= \sum_{n \geq 1} na_n X^{n-1} \\
    &=\sum_{n=1}^N na_n X^{n-1} && \forall N \geq \deg(P) \\
    &=\sum_{p=0}^{N-1} (p+1)a_{p+1} X^{p} && \forall N \geq \deg(P) \\
  \end{align}
\end{defdef}
...
\end{prop}
\begin{proof}
  Si $\deg(P)>0$ on écrit $P$ sous la forme $P=\sum_{n=0}^N a_n X^n$ avec $N=\deg(P) \in \N^*$ et tel que $a_N \neq 0$. Alors $P'=\sum_{p=0}^{N-1} (p+1)a_{p+1} X^{p}$ et de plus $\deg(P') \leq N-1$ $(N-1+1)a_{N-1+1}=Na_N \neq 0$. Le degré vaut donc $\deg(P') = N-1=\deg(P)-1$.

  Si $\deg(P) \leq 0$, alors $P$ est un polynôme constant et son polynôme dérivée est nul. Alors son degré vaut $\deg(P')=-\infty$.
\end{proof}
...
\end{prop}
\begin{proof}
  On note $P=\sum_{n \geq 0} a_n X^n$ et $Q=\sum_{n \geq 0} b_n X^n$. Alors $\lambda P+Q = \sum_{n \geq 0} (\lambda a_n+b_n) X^n$. Par définition du polynôme dérivé, on a
  \begin{equation}
    (\lambda P+Q)' = \sum_{n \geq 1} n(\lambda a_n+b_n) X^{n-1}
  \end{equation}
  D'où
  \begin{align}
    (\lambda P+Q)' &= \lambda \sum_{n \geq 1} n a_n X^{n-1} + \sum_{n \geq 1} n b_n X^{n-1}\\
    &=\lambda P' +Q'
  \end{align}
\end{proof}
...
\end{prop}
\begin{proof}
  On note $P=\sum_{n \geq 0} a_n X^n$ et $Q=\sum_{k \geq 0} b_k X^k$. Alors
  \begin{equation}
    PQ = \left(\sum_{n \geq 0} a_n X^n\right) \left(\sum_{k \geq 0} b_k X^k\right) = \sum_{n \geq 0} \sum_{k \geq 0} a_nb_k X^{n+k}
  \end{equation}
  La dérivation est linéaire donc~:
  \begin{equation}
    (PQ)' = \sum_{n \geq 0} \sum_{k \geq 0} a_nb_k (X^{n+k})'
  \end{equation}
  ...
  Alors
  \begin{align}
    (PQ)' &= \sum_{n \geq 0} \sum_{k \geq 0} a_nb_k ((X^n)'X^k +(X^k)'X^n) \\
    &= \sum_{n \geq 0} a_n (X^n)' \cdot \sum_{k \geq 0} b_k X^k + \sum_{n \geq 0} a_n X^n \cdot \sum_{k \geq 0} b_k (X^k)' \\
    &= \left(\sum_{n \geq 0} a_n X^n\right)' Q + P \left(\sum_{k \geq 0} b_k X^k\right)' \\
    &=P'Q+PQ'
  \end{align}
\end{proof}
...
\begin{proof}
  ...
  \begin{equation}
    \deg(P^{(k)}) = \deg(P)-k = 0
  \end{equation}
  Ainsi $P^{(k+1)}$ est le polynôme nul. Alors si on dérivent encore plus, c'est-à-dire si $\deg(P) \leq n$ alors $P^{(n+1)}=0$.

  Montrons la deuxième implication par contraposée~: si maintenant $\deg(P) \geq n+1$ alors par récurrence ``immédiate''
  \begin{align}
    \deg(P^{(n)}) &\geq n+1-n = 1 \\
    \deg(P^{(n+1)}) &\geq 0
  \end{align}
...
\begin{theo}[Formule de Taylor pour les polynômes]
  ...
  \begin{equation}
    P = \sum_{n=0}^N \frac{P^{(n)}(a)}{n!} (X-a)^n \quad \forall N \geq \deg(P)
  \end{equation}
\end{theo}
\begin{proof}
  ...
  \begin{align}
    \varphi(\lambda P +Q) &= \sum_{n \geq 0} \frac{(\lambda P+Q)^{(n)}(a)}{n!}(X-a)^n\\
 &= \sum_{n \geq 0} \frac{\lambda P^{(n)}(a) +Q^{(n)}(a)}{n!}(X-a)^n\\
 &= \lambda \sum_{n \geq 0} \frac{P^{(n)}(a)}{n!}(X-a)^n + \sum_{n \geq 0} \frac{Q^{(n)}(a)}{n!}(X-a)^n \\
...
\begin{proof}
  ...
  \item Si $r \geq 2$ alors $P'(\alpha) = 0$ et donc $\alpha$ est racine de $P'$. De plus
    \begin{align}
