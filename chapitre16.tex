...
\begin{defdef}
  On appelle projection tout endomorphisme \(p \in \Endo{E}\) tel que \(p \circ p=p\).
\end{defdef}
...
\end{prop}
\begin{proof}
  Si \(p(x)=x\) alors \(x \in \Image(p)\), c'est trivial. Soit maintenant \(x \in \Image(p)\), alors il existe \(y \in E\) tem que \(x=p(y)\) et \(p(x)=p(p(y))=p(y)=x\).
\end{proof}
...
\end{prop}
\begin{proof}
  L'inclusion \(\{0\} \subset \Inv(u) \cap \Opp(u)\) est triviale puisque ce sont deux sous-espaces vecteoriels donc l'intersection l'est aussi. Soit \(x \in \Inv(u) \cap \Opp(u)\) alors \(-x=u(x)=x\) donc \(2x=0\) et comme dans \(\K\) \(2\neq 0\) on a bien \(x=0\), alors \(\Inv(u) \cap \Opp(u) \subset \{0\}\).
